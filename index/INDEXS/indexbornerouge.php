...
.dropdown:hover .dropbtn {background-color: rgb(143, 0, 105);}
    ...
    <div class="produit">
        <div class="container">
            ...
            <?php
                    $req = "SELECT name, price, quantité, codepromo, description FROM products WHERE id=2";
                    $ORes = $bdd->query($req);
                    while($produit = $ORes->fetch())
                    {
            ?>
                <img class="imageproduit" src="img/360x360-rose.png">
                ...
                <h4 class="titreproduit"><?php echo $produit->name; ?>
                  ...
                  <a href="index.php?page=reservation&idBorne=2"><button class="dropbtn">commander</button></a>
                </div>
                <BR><BR>
                    <p class="textproduit aaaa" >disponible en rouge :</p>
                    <a href="index.php?page=indexbornesnoir"><img class="disponibilite" src="img/born2.png"></a>
                </h4>
        </div>
    </div>
    <!----------------formulaire PRODUIT----------------->
